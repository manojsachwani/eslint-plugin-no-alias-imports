...
module.exports = {
  ...
  create(context) {
    ...
    /**
     * Check if a module path is allowed (in allowlist)
     * @param {string} modulePath - The import/require path
     * @returns {boolean} - True if it's in the allowlist
     */
    function isAllowed(modulePath) {
      return allowlist.some((allowed) => {
        // Support exact matches and pattern matches
        if (allowed.includes("*")) {
          const regex = new RegExp(allowed.replace(/\*/g, ".*"));
          return regex.test(modulePath);
        }
        return modulePath === allowed || modulePath.startsWith(allowed);
      });
    }
    ...
    return {
      ...
      ImportDeclaration(node) {
        ...
        if (isAliasImport(modulePath) && !isAllowed(modulePath)) {
